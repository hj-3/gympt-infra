...
**위치**: `terraform/modules/boundary`

**목적**: HashiCorp Boundary를 EC2 단일 인스턴스에 설치하여 Zero-Trust Bastion을 구성한다. EKS, RDS 등 내부 리소스에 대한 접근을 Boundary Controller/Worker를 통해 중앙 관리한다.

**구성 요소**:
- EC2 인스턴스 (Amazon Linux 2023, 기본 `t3.medium`)
- PostgreSQL 15 (EC2 내부 — Boundary 백엔드 DB)
- HashiCorp Boundary Controller + Worker (동일 인스턴스)
- KMS 키 3개: `root`, `worker-auth`, `recovery`
- IAM Role: SSM Session Manager + KMS 접근
- Security Group: 포트 9200(API/UI), 9201(Cluster), 9202(Proxy)

**포트 용도**:

| 포트 | 용도 |
|------|------|
| 9200 | Boundary API 및 Web UI |
| 9201 | Controller ↔ Worker 통신 (Cluster) |
| 9202 | 클라이언트 세션 프록시 (Proxy) |
...
| 변수 | 설명 | 타입 | 기본값 | 필수 |
|------|------|------|--------|------|
| `vpc_id` | Boundary EC2를 배치할 VPC ID | string | - | Y |
| `public_subnet_id` | 배치할 퍼블릭 서브넷 ID | string | - | Y |
| `instance_type` | EC2 인스턴스 타입 | string | `t3.medium` | N |
| `boundary_version` | 설치할 Boundary 버전 | string | `0.17.1` | N |
| `db_password` | PostgreSQL boundary 유저 패스워드 | string (sensitive) | - | Y |
| `allowed_cidr_blocks` | SG ingress 허용 CIDR | list(string) | `["0.0.0.0/0"]` | N |

**출력값**:

| 이름 | 설명 |
|------|------|
| `public_ip` | Boundary EC2 퍼블릭 IP |
| `api_endpoint` | Boundary API/UI 엔드포인트 (`http://<IP>:9200`) |
| `instance_id` | EC2 인스턴스 ID |
| `kms_root_key_arn` | Root KMS 키 ARN |
| `kms_worker_auth_key_arn` | Worker-auth KMS 키 ARN |
| `kms_recovery_key_arn` | Recovery KMS 키 ARN |
| `security_group_id` | Boundary Security Group ID |
...
# Boundary 초기화 결과 확인 (초기 admin 비밀번호 포함)
cat /var/log/boundary-init.log

# 설치 전체 로그
cat /var/log/boundary-setup.log
...
**관련 파일**:
- `terraform/modules/boundary/main.tf` — EC2, KMS, IAM, SG 리소스
- `terraform/modules/boundary/templates/setup-boundary.sh.tpl` — user data 스크립트 (PostgreSQL + Boundary 설치/설정/서비스 등록)
