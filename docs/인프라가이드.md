...
### Boundary 접근 제어 아키텍처 (2026-05-29 추가)

Zero-Trust Bastion으로 HashiCorp Boundary를 사용한다. 운영자는 SSH/kubectl 직접 접근 없이 Boundary 세션을 통해 내부 리소스에 접근한다.

```
운영자 (로컬)
    │  boundary connect CLI / Desktop App
    ▼
┌──────────────────────────────────────────┐
│          Public Subnet                    │
│                                           │
│  ┌──────────────────────────────────┐    │
│  │  Boundary EC2 (t3.medium)        │    │
│  │  ├── Controller  :9200 (API/UI)  │    │
│  │  │               :9201 (Cluster) │    │
│  │  ├── Worker      :9202 (Proxy)   │    │
│  │  └── PostgreSQL  :5432 (내부)    │    │
│  └──────────────────────────────────┘    │
...
**AWS 리소스**:
- EC2: `gympt-prod-boundary` (Amazon Linux 2023, t3.medium)
- KMS: `gympt-prod-boundary-root`, `gympt-prod-boundary-worker-auth`, `gympt-prod-boundary-recovery`
- IAM Role: SSM Session Manager + KMS 접근
- Security Group: 포트 9200/9201/9202 제어
